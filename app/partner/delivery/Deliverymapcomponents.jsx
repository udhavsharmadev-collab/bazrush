...
// ── Leaflet Map ───────────────────────────────────────────────────────────────
export function DeliveryMap({ fromLat, fromLng, toLat, toLng, toLabel, toEmoji, toColor }) {
  const ref = useRef(null);
  const inst = useRef(null);

  useEffect(() => {
    if (!ref.current || !fromLat || !toLat) return;
    ...
    const boot = () => {
      ...
      const map = L.map(ref.current).setView(
        [(fromLat + toLat) / 2, (fromLng + toLng) / 2], 14
      );
      inst.current = map;
      ...
      L.marker([fromLat, fromLng], { icon: mk("🛵", "#7c3aed") }).addTo(map).bindPopup("<b>You</b>");
      L.marker([toLat, toLng], { icon: mk(toEmoji, toColor) }).addTo(map).bindPopup(`<b>${toLabel}</b>`);
      L.polyline([[fromLat, fromLng], [toLat, toLng]], {
        color: "#7c3aed", weight: 3, dashArray: "8 5", opacity: 0.8,
      }).addTo(map);
    ...
    if (window.L) { boot(); }
      ...
      const iv = setInterval(() => { if (window.L) { clearInterval(iv); boot(); } }, 80);
    }
  ...
  return <div ref={ref} style={{ height: 220, borderRadius: 16, overflow: "hidden", zIndex: 0, border: "1px solid #e9d5ff" }} />;
}
      ...
      {steps.map((s, i) => (
        <div key={i} className="flex items-center flex-1">
          <div className="flex flex-col items-center">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs border-2 transition-all ${i <= step ? "border-purple-500 bg-purple-50" : "border-gray-200 bg-gray-50"}`}>
              {s.icon}
            </div>
            <p className={`text-[9px] font-bold mt-0.5 whitespace-nowrap ${i <= step ? "text-purple-600" : "text-gray-300"}`}>{s.label}</p>
          </div>
          {i < steps.length - 1 && (
            <div className={`flex-1 h-0.5 mb-3 mx-1 rounded-full ${i < step ? "bg-purple-400" : "bg-gray-200"}`} />
          )}
        </div>
      ))}
    </div>

...
<div className="mx-4 mt-4">
  ...
  >
    ...
    <span className="absolute bottom-3 right-3 text-[10px] font-bold px-2 py-1 rounded-full bg-black/30 text-white backdrop-blur-sm">
      🔍 Tap to zoom
    </span>
  </div>
...
{zoomOpen && (
  <div
    ...
    onClick={() => { setZoomScale(1); setZoomOpen(false); }}
  >
    <button
      className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/20 text-white text-xl flex items-center justify-center backdrop-blur z-10"
      onClick={() => { setZoomScale(1); setZoomOpen(false); }}
    >✕</button>

    <p className="absolute top-4 left-1/2 -translate-x-1/2 text-white/60 text-xs font-semibold">
      Tap image to zoom · Pinch to zoom
    </p>
    ...
    <img
      ...
      onClick={(e) => {
        e.stopPropagation();
        setZoomScale(s => s >= 2.5 ? 1 : parseFloat((s + 0.75).toFixed(2)));
      }}
    />

    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3">
      <button
        className="w-10 h-10 rounded-full bg-white/20 text-white text-xl font-black flex items-center justify-center backdrop-blur active:scale-90 transition-all"
        onClick={(e) => { e.stopPropagation(); setZoomScale(s => parseFloat(Math.max(1, s - 0.75).toFixed(2))); }}
      >−</button>
      <span className="text-white text-sm font-bold bg-white/20 backdrop-blur px-3 py-1.5 rounded-full min-w-[52px] text-center">
        {Math.round(zoomScale * 100)}%
      </span>
      <button
        className="w-10 h-10 rounded-full bg-white/20 text-white text-xl font-black flex items-center justify-center backdrop-blur active:scale-90 transition-all"
        onClick={(e) => { e.stopPropagation(); setZoomScale(s => parseFloat(Math.min(3, s + 0.75).toFixed(2))); }}
      >+</button>
    </div>
  </div>
)}

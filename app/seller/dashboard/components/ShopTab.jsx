...
const ShopTab = ({ seller }) => {
  ...
  useEffect(() => {
    if (!isEditing || !editingShopId) return;

    const check = () => {
      const currentIsOpen = isShopOpenNow(shopData.timing, shopData.overrideUntil, shopData.overrideStatus);
      if (prevIsOpenRef.current === null) {
        prevIsOpenRef.current = currentIsOpen;
        return;
      }
      if (prevIsOpenRef.current !== currentIsOpen) {
        prevIsOpenRef.current = currentIsOpen;
        handleSaveShop();
      }
    };
  ...
  }, [isEditing, editingShopId, shopData.timing, shopData.overrideUntil, shopData.overrideStatus]);
  ...
  const handleEditShop = async (shop) => {
    const res = await fetch(`/api/sellers?phoneNumber=${encodeURIComponent(seller?.phoneNumber)}`);
    const data = await res.json();
    const freshShop = data?.seller?.shops?.find(s => s.id === shop.id) || shop;
